...
function renderToolPart(part: Extract<UiPart, { type: `tool-${string}` }>, index: number) {
  ...
  return (
    <div key={`${toolName}-${index}`} className="rounded-md border border-[var(--color-border)] bg-[var(--color-card)]/60 p-2 text-xs">
      <div className="mb-1 font-medium">Tool: {toolName}</div>
      <div className="text-[var(--color-muted-foreground)]">State: {part.state ?? "completed"}</div>
      {"output" in part && part.output ? (
        <pre className="mt-2 overflow-x-auto whitespace-pre-wrap rounded bg-[var(--color-background)] p-2 text-[11px]">
...
export function Message({ message }: { message: UiMessage }) {
  ...
  return (
    <div className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}>
      <Card className={cn("max-w-[80%] text-sm", message.role === "user" ? "bg-[var(--color-primary)]/15" : "")}>
        {text ? <MessageResponse>{text}</MessageResponse> : <p className="whitespace-pre-wrap leading-relaxed">...</p>}
        {reasoningParts.length > 0 ? (
          <details className="mt-2 text-xs">
            <summary className="cursor-pointer text-[var(--color-muted-foreground)]">Reasoning</summary>
...
export function MessageResponse({ children }: { children: string }) {
  return <p className="whitespace-pre-wrap text-sm leading-relaxed [&_code]:rounded [&_code]:bg-[var(--color-background)] [&_code]:px-1">{children}</p>;
}
