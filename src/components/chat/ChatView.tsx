...
export function ChatView({
  ...
  function inferCouncilScopeFromText(text: string): { kind: "postcode"; postcode: string } | { kind: "area"; area: string } | { kind: "national" } {
    const postcodeMatch = text.match(UK_POSTCODE_REGEX);
    if (postcodeMatch?.[1]) {
      return { kind: "postcode", postcode: postcodeMatch[1].replace(/\s+/g, "").toUpperCase() };
    }
    const constituencyMatch = text.match(/\bconstituency\s+([a-z0-9][a-z0-9\s'&-]{2,})/i);
    if (constituencyMatch?.[1]) {
      return { kind: "area", area: constituencyMatch[1].trim() };
    }
  ...
  async function submitPrompt(payload: PromptInputSubmitPayload) {
    ...
    if (payload.mode === "council") {
      ...
      const scope = inferCouncilScopeFromText(text);
