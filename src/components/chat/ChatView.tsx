...
function buildAutoChatTitle(prompt: string): string | null {
  const normalized = prompt.replace(/\s+/g, " ").trim();
  if (!normalized) return null;
  const clean = normalized.replace(/^["'`]+|["'`]+$/g, "");
  if (!clean) return null;
  if (clean.length <= AUTO_CHAT_TITLE_MAX_LENGTH) return clean;
  return `${clean.slice(0, AUTO_CHAT_TITLE_MAX_LENGTH - 3).trimEnd()}...`;
}
...
export function ChatView({
  ...
  async function submitPrompt(payload: PromptInputSubmitPayload) {
    ...
    const autoTitle = buildAutoChatTitle(text);
    if (autoTitle && isAutoGeneratedTitle(conversation?.title)) {
      onRenameConversation(ensuredConversationId, autoTitle);
    }
    const toolPrefix =
